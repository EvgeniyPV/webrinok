...
class DUP_PRO_Dup_Archive extends DUP_PRO_Archive
{
    ...
    public static function create(DUP_PRO_Archive $archive, DUP_PRO_Build_Progress $buildProgress)
    {
        try {
            ...
            DupArchiveEngine::init(new DUP_PRO_Dup_Archive_Logger(), $archive->getTargetRootPath());
            ...
            try {
                if ($buildProgress->custom_data == null) {
                    $createState = DUP_PRO_Dup_Archive_Create_State::createNew(
                        $archive->Package,
                        $archivePath,
                        $compressDir,
                        self::WorkerTimeInSec,
            ...
            if ((!$skipArchiveFinalization) && ($createState->working == false)) {
                if (!$buildProgress->installer_built) {
                    ...
                    if ($expandStateEntity == null) {
                        ...
                        $expandStateEntity->validationType         = DupArchiveExpandState::VALIDATION_STANDARD;
                        $expandStateEntity->working                = true;
                        ...
                        // add index file
                        $expandStateEntity->expectedFileCount      = 1 + count($scanReport->ARC->Files) - $createState->skippedFileCount + $package->Installer->numFilesAdded;
